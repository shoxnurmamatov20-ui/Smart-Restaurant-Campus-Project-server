...
Smart Restaurant Campus Phase 1+ targets **10–50 Telegram bots** that integrate with the web platform. Each bot serves a specific audience (mehmon, ofitsiant, oshpaz, kuryer, menejer, egasi, yetkazib beruvchi, va h.k.) or a specific module workflow. All bots share:
...
- Webhook URL pattern: `https://restaurant-campus.uz/tg/webhook/{bot_key}/{secret_token}`
...
1. `guest` — mehmon: menyu, buyurtma, bron, bonus balansi
2. `parent_bot` — ota-ona: farzand holati, kelish-ketish, baholar, to'lovlar
3. `kitchen` — oshpaz: yangi chiptalar, taymer, stop-list
...
9. `feedback` — mehmon: taom va xizmat bahosi, shikoyat
10. `supplier` — yetkazib beruvchi: xarid arizalari, yetkazish, hisob-kitob

40 future bots (Phase 2+): stock alerts, waste control, HACCP checks, shift swaps, payroll, staff training, recruiting, equipment maintenance, energy monitoring, security incidents, birthday offers, win-back campaigns, catering requests, corporate accounts, gift cards, external review watch, AI menu assistant, nutrition, allergen warnings, queue management, aggregator orders, delivery zones, courier dispatch, order tracking, cash anomalies, fiscal errors, supplier debt, budget alerts, per-branch channels (×8), concept bots, franchise and internal audit — totalling 50.
...
- WebApp button in bot → opens https://restaurant-campus.uz/tg-app/{module}?tgWebApp=true
...
- User is now logged in as their Telegram-linked platform user, no separate password
