/**
 * Where the console keeps the reader's language.
 *
 * A cookie rather than a URL segment: the console is one deployment per tenant
 * and its routes are the module aliases the RBAC layer already uses, so
 * prefixing every one of them with a locale would change the permission
 * surface for a preference. A cookie also survives a full reload, which a
 * `useState` would not, and — the reason it is a cookie and not localStorage —
 * the server can read it, so a page rendered on the server comes back in the
 * right language rather than flashing Uzbek first.
 *
 ...
 */
export const LOCALE_COOKIE = 'restaurant-campus-locale';
