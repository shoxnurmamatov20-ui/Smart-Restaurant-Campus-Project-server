...
/**
 * `/roles` — its own title, description and canonical.
 *
 ...
 * `roles-board.tsx` keeps the state the design's screen needs.
 */
const base: Metadata = {
  title: { absolute: 'Kim uchun — to’qqiz rol, to’qqiz ekran' },
  description:
 ...
 *
 * A `metadata` constant cannot: it is evaluated once and would give `/uz/roles`
 * and `/ru/roles` the same canonical, which tells a crawler the Russian
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const { title, description } = pageMeta(locale, 'roles');
  ...
  return {
    ...
    alternates: await localeAlternates('/roles'),
  };
}
